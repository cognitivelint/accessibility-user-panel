...
import { maxSeverity } from "../util/severity.js";
...
function clusterKey(finding: Finding): string {
...
export function clusterFindings(findings: Finding[]): Cluster[] {
  ...
  return [...groups.entries()].map(([key, members]) => {
    ...
    const confidence = members.sort((a, b) => CONFIDENCE_RANK[b.confidence] - CONFIDENCE_RANK[a.confidence])[0]?.confidence ?? "low";
    return {
